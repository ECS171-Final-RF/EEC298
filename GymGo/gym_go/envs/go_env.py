...
class GoEnv(gym.Env):
    ...
    def step_batch(self, state, action):
        ...
        actions = np.array([action])
        next_states, next_group_maps = GoGame.get_batch_next_states(state, actions, self.group_map)
        next_state, next_group_map = next_states[0], next_group_maps[0]
        next_done = GoGame.get_game_ended(next_state)
        return np.copy(next_state), self.get_reward_batch(next_state, next_done), next_done, self.get_info_batch(next_state)
    ...
    def game_ended_batch(self, done):
        return done
    ...
    def get_info_batch(self, state):
        """
        :return: Debugging info for the state
        """
        return {
            'prev_player_passed': GoGame.get_prev_player_passed(state),
            'turn': 'b' if GoGame.get_turn(state) == GoEnv.govars.BLACK else 'w',
    ...
    def get_winning_batch(self, state):
        """
        :return: Who's currently winning in BLACK's perspective, regardless if the game is over
        """
        return GoGame.get_winning(state)
    ...
    def get_winner_batch(self, state, done):
        """
        Get's the winner in BLACK's perspective
        :return:
        """

        if self.game_ended_batch(done):
            return self.get_winning_batch(state)
        else:
            return 0
    ...
    def get_reward_batch(self, state, done):
        '''
        Return reward based on reward_method.
        heuristic: black total area - white total area
        real: 0 for in-game move, 1 for winning, 0 for losing,
        ...
        '''
        if self.reward_method == RewardMethod.REAL:
            return self.get_winner_batch(state, done)
